...
import { handleDownloadSlip } from '../services/verification'; // PDF Service
...
export default function AuthPage() {
  ...
  return (
    <div className={`min-h-screen w-screen flex items-center justify-center relative transition-colors duration-500 py-6 lg:py-0 ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-slate-200 text-slate-900'}`}>
      ...
      {/* Unified Success Modal (After Registration) */}
      {authSuccess && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm p-8 rounded-[40px] shadow-2xl text-center border border-white/10 relative overflow-hidden">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tighter mb-2 dark:text-white">{authSuccess.title}</h2>
            ...
            <div className="flex flex-col gap-3">
              {/* FIXED: Passing data for PDF Slip */}
              <button 
                type="button"
              ...
              <button 
                onClick={() => { setAuthSuccess(null); setIsSignup(false); }} 
      ...
      <div className={`relative z-20 w-full px-4 transition-all duration-700 ${isSignup ? 'max-w-6xl' : 'max-w-4xl'}`}>
        <div className={`w-full flex flex-col lg:flex-row rounded-[40px] shadow-2xl overflow-hidden border backdrop-blur-2xl transition-all duration-500
          ...
          <div className={`lg:w-1/3 p-10 flex flex-col items-center justify-center text-center border-b lg:border-b-0 lg:border-r 
            ...
            {!isSignup && (
              <div className="w-full max-w-[240px] space-y-3">
                <label className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-400 block text-center">Resident Tracker</label>
                <input 
                  type="text" value={trackingNum} onChange={(e) => handleTrackSearch(e.target.value)} placeholder="BSB-2026-XXXX"
                  ...
                  <div className="mt-4 p-4 rounded-2xl bg-white/50 dark:bg-black/30 border border-white/20 animate-in fade-in slide-in-from-top-2 text-left">
                    <div className="flex items-center justify-between mb-2 gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase border 
                        ${searchResult.status === 'Verified' ? 'bg-emerald-100 text-emerald-600 border-emerald-200' : 'bg-orange-100 text-orange-600 border-orange-200'}`}>
                        {searchResult.status}
                      </span>
                    </div>
                    <p className="text-sm font-black text-slate-800 dark:text-white leading-tight mb-1">{searchResult.message}</p>
                    <p className="text-[10px] font-medium text-slate-500 italic">Resident: {searchResult.name}</p>
                    
                    {/* Optional: Add download button also here in tracker
                    <button 
                      onClick={() => handleDownloadSlip({
                      ...
                      className="mt-3 w-full py-2 bg-slate-800 text-[9px] text-white font-bold rounded-lg uppercase tracking-widest flex items-center justify-center gap-1 hover:bg-black transition-all"
                    >
                      <Download size={10} /> Download Slip
                    </button> */}
                  </div>
                )}
              </div>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            {/* Form Footer */}
            <div className={`p-8 lg:px-12 border-t flex flex-col sm:flex-row items-center justify-between gap-6 ${isDarkMode ? 'bg-black/40 border-white/5' : 'bg-slate-50/80 border-black/5'}`}>
              <button 
                type="button" 
              ...
              >
                {isSignup ? <><ArrowLeft size={12}/> Back to Login</> : "New Resident? Signup here →"}
              </button>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .full-input-sm { 
          ...
          border-radius: 20px; background: ${isDarkMode ? '#0f172a' : '#fff'}; color: ${isDarkMode ? '#fff' : '#0f172a'}; 
