...
const Header = ({ toggleSidebar }) => {
  ...
  return (
    <header className="h-[89px] bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center px-6 sticky top-0 z-10 transition-colors duration-300">
      ...
      {/* Center Date/Time - Hidden on small screens for better spacing */}
      ...
      <div className="ml-auto flex items-center gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <div
              className={`
                bg-white dark:bg-emerald-400 w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 flex items-center justify-center
                ${isDark ? "translate-x-7" : "translate-x-0"}
              `}
            >
              {isDark ? (
                <Moon size={12} className="text-emerald-950" />
              ) : (
        ...
        <div className="flex items-center gap-3 border-l pl-6 border-slate-200 dark:border-slate-700">
          ...
          <div
            className={`w-10 h-10 rounded-xl shadow-sm flex items-center justify-center text-xs font-bold ring-2 ring-white dark:ring-slate-800 ${getAvatarColor(
              user.name
            )}`}
          >
