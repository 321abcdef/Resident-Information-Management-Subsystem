...
const handleAction = async (status) => {
  const actionText =
    ...
    status === 'For Verification' ? 'set for PHYSICAL VERIFICATION' :
    'DISAPPROVE';

  if (!window.confirm(`Are you sure you want to ${actionText} this submission?`)) return;

  // Tawagin ang updateStatus mula sa useVerification hook
  const result = await updateStatus(selectedRes.id, status);

  if (result.success) {
    if (status === 'Verified') {
      // FIXED MAPPING: Dito natin kukunin ang data mula sa residentData ng backend
      const details = {
        name: selectedRes.name,
        id:   result.residentData?.id,   // Barangay ID
        user: result.residentData?.user, // Username
        pass: result.residentData?.pass, // Plain text temporary password
      };
    ...
    alert(`Error: ${result.message || 'Database update failed.'}`);
  }
};
  ...
  return (
    <div className="font-sans min-h-screen pb-20 px-4 md:px-0 relative">
      ...
      {lastVerified && !showSuccess && (
        <button
          onClick={() => { setAccountDetails(lastVerified); setShowSuccess(true); }}
      ...
      {view === 'list' ? (
        <div className="animate-in fade-in duration-500 space-y-8">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            ...
            {loading ? (
              <div className="p-10 text-center text-slate-400 italic">Updating queue...</div>
            ) : (
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200 dark:border-slate-800 pb-8">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-2 font-bold uppercase tracking-widest">
          Status: {data?.status} | Submitted: {data?.date}
        </p>
      </div>
      <VerificationActions
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="bg-slate-900 p-8 rounded-2xl text-white border-t-4 border-emerald-500 h-fit">
        ...
        <div className="space-y-6">
          ...
          <InfoField label="House No." val={data?.details?.houseNumber} />
...
const IDCard = ({ label, src, onClick }) => (
  <div className="space-y-3">
    ...
    <div
      onClick={onClick}
      className="group relative aspect-video bg-slate-100 rounded-xl overflow-hidden cursor-zoom-in border border-slate-200"
    >
